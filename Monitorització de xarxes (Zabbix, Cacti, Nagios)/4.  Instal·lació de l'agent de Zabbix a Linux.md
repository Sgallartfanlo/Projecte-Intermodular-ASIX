...
*Execució de `sudo -s` per ser root, descàrrega del paquet `.deb` amb `wget`, instal·lació amb `dpkg` i finalment `apt install zabbix-agent`.*
...
*Obertura del fitxer de configuració amb `nano /etc/zabbix/zabbix_agentd.conf`.*
...
*Obertura dels ports `10050` i `10051` mitjançant `ufw allow`.*

![Reinici del servei](img/ins_lin/Restart.png)

*Reinici del servei amb `systemctl restart zabbix-agent.service`.*

![Activació de l'inici automàtic](img/ins_lin/Enable.png)

*Configuració del servei perquè s'iniciï automàticament amb el sistema.*

![Comprovació de l'estat](img/ins_lin/Estat.png)

*Comprovació que el servei està actiu i funcionant correctament.*
