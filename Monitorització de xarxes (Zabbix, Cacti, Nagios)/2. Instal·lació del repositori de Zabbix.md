...
- zabbix-server-mysql → servidor de Zabbix amb suport per MySQL.
- zabbix-frontend-php → interfície web de gestió.
- zabbix-apache-conf → configuració d’Apache per al frontend.
- zabbix-sql-scripts → scripts per crear la base de dades.
- zabbix-agent → agent que recull dades del sistema.
